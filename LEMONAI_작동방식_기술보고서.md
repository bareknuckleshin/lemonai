...
LemonAI 데이터 영속 계층은 Sequelize ORM + SQLite 조합 기반 구성.
주 DB 파일은 `getFilepath('data', 'database.sqlite')` 경로 해석 결과 위치에 생성.
스키마 반영은 `Model.sync({ alter: true })` 중심의 런타임 동기화 방식.

### 3.2 초기화 및 기본 데이터

앱 시작 시 `src/models/sync.js`에서 테이블 sync와 기본 데이터 주입 수행.
기본 데이터 소스는 `public/default_data/default_platform.json`, `public/default_data/default_search_provider.json` 중심 구성.
운영 중 버전 보정성 업데이트는 동일 파일의 `dataUpdate()` 단계에서 수행.

### 3.3 저장 계층 이중 구조 요약

LemonAI의 경험/상태 저장은 실무적으로 장기 영속 계층과 단기 실행 계층의 이중 구조.

- 장기 영속 계층: SQLite DB 테이블군(Conversation, Message, Task, Knowledge, FileVersion 등) 저장 구조.
- 단기 실행 계층: LocalMemory 기반 task 루프 메시지/오류 피드백 누적 구조.

### 3.4 언제 어떤 저장소를 사용하는지 기준

- 대화 이력, 지식, 설정, 파일 버전처럼 다음 세션에도 유지되어야 하는 정보는 SQLite DB 사용 기준.
- 현재 task 실행 중 즉시 보정에만 필요한 문맥은 LocalMemory 사용 기준.
- 실행 실패 피드백은 LocalMemory에 먼저 누적되어 재시도 품질 보정 수행, 반복적으로 의미가 검증된 규칙은 Knowledge Reflection 경로를 통해 DB 지식으로 승격되는 작동 원리.
